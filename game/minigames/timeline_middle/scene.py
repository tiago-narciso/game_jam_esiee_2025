...
from ...config import WIDTH, HEIGHT, WHITE, DARK, ACCENT, GOOD, BAD, GRAY, IMG_DIR
...
class TimelineMiddleScene(Scene):
    ...
    def draw(self, screen):
        screen.fill(DARK)
        blit_text_center(screen, self.title_font.render("Stoppe au milieu de l'histoire (iPhone)", True, WHITE), 56)
        hint = "ESPACE (ou clic) pour ARRÊTER • M: Menu" if self.state == "scrolling" else "R pour rejouer • clic pour continuer"
        blit_text_center(screen, self.ui_font.render(hint, True, GRAY), 86)
        ...
        cursor_y0 = 120
        cursor_y1 = HEIGHT - 60
        pygame.draw.line(screen, ACCENT, (WIDTH // 2, cursor_y0), (WIDTH // 2, cursor_y1), 2)
        ...
        if self.state == "stopped":
            ...
            if self.result == "win":
                ...
                t2 = self.ui_font.render("Tu as stoppé au milieu de l'histoire.", True, WHITE)
            else:
                t1 = render_not_center_message(self.title_font)
                t2 = self.ui_font.render("Ce n'était pas le milieu exact.", True, WHITE)
            blit_text_center(screen, t1, HEIGHT // 2 - 8)
            blit_text_center(screen, t2, HEIGHT // 2 + 24)
            blit_text_center(screen, self.ui_font.render(f"Score: {getattr(self, 'score', 0)}", True, WHITE), HEIGHT // 2 + 48)
